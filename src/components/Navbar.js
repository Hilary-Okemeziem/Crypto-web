...
const Navbar = () => {
  ...
  return (
    <div className='rounded-div flex items-center justify-between h-20 font-bold'>
        <Link to='/'>
            <h1 className='text-2xl'>CryptoExpress</h1>
        </Link>
        ...
        {user?.email ? (
          <div>
            ...
            <button onClick={handleSignOut} className='bg-button text-btnText px-5 py-2 ml-2 rounded-2xl shadow-lg hover:shadow-2xl hidden sm:inline'>Sign Out</button>
            <button onClick={handleSignOut} className='sm:hidden'>Sign Out</button>
          </div>
        ) : (
          <div className='hidden md:block'>
